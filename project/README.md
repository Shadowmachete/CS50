...
Next, on to _"/search"_. Upon clicking the "Search" in the navigation bar, the user will be shown the **search.html** template file. This file will first prompt the user to fill up various fields to narrow the search. The user can choose to provide an id, name, primary type, secondary type, generation number and whether or not the pokemon is a legendary.