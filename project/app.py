...
def search():
    if request.method == "POST":
        ...
        if order != '':
            if orderdir != '':
                data = db.execute(f'SELECT * FROM pokemon WHERE id LIKE ? AND Name LIKE ? AND `Type 1` LIKE ? AND `Type 2` LIKE ? AND Generation LIKE ? AND Legendary LIKE ? ORDER BY "{order}" {orderdir}', "%" + id + "%", "%" + name + "%", "%" + type1 + "%", "%" + type2 + "%", "%" + gen + "%", "%" + isLegend + "%")
            else:
                data = db.execute(f'SELECT * FROM pokemon WHERE id LIKE ? AND Name LIKE ? AND `Type 1` LIKE ? AND `Type 2` LIKE ? AND Generation LIKE ? AND Legendary LIKE ? ORDER BY "{order}"', "%" + id + "%", "%" + name + "%", "%" + type1 + "%", "%" + type2 + "%", "%" + gen + "%", "%" + isLegend + "%")
        else:
...
def pokemonData():
    if request.method == "POST":
        ...
        try:
            ...
            return render_template("search.html", error="Please choose a valid pokemon")
        if id <= 0:
            return render_template("search.html", error="Please choose a valid pokemon")
